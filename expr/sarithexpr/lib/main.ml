...
let rec typecheck = function
  ...
  | And(e1, e2) -> 
      (match (eval e1, eval e2) with
      | (Bool _, Bool _) -> BoolT
      | (Bool _, _) -> raise (TypeError (string_of_expr e2 ^ " has type Nat, but type Bool was expected"))
      | (_, Bool _) -> raise (TypeError (string_of_expr e1 ^ " has type Nat, but type Bool was expected"))
      | _ -> raise (TypeError (string_of_expr e1 ^ " has type Nat, but type Bool was expected")))
  | Or(e1, e2) -> 
      (match (eval e1, eval e2) with
      | (Bool _, Bool _) -> BoolT
  ...
  | Succ(e1) -> 
      (match eval e1 with
      | Nat _ -> NatT
      | _ -> raise (TypeError (string_of_expr e1 ^ " has type Bool, but type Nat was expected")))
  | Pred(e1) -> 
      (match eval e1 with
      | Nat n1 -> if n1 > 0 then NatT else raise (TypeError (string_of_expr e1 ^ " has type Int or equals 0, but type Nat was expected"))
      | _ -> raise (TypeError (string_of_expr e1 ^ " has type Bool, but type Nat was expected")))
  | IsZero(e1) -> 
      (match eval e1 with
      | Nat _ -> BoolT
